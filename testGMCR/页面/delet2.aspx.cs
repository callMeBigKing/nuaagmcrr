...
public partial class 页面_delet2 : System.Web.UI.Page
{
    GMCR model = new GMCR();
    ...
    {  
        ...
        int[][] delet2_matrix = new int[row][];
        for (int i = 0; i < row; i++)
        {
            delet2_matrix[i] = new int[col];
        }
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < col; j++)
            {
                ...
                delet2_matrix[i][j] = Convert.ToInt16(label.Text);

            }
        }

        model.delet_state2(delet2_matrix);
